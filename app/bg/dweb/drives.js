...
import datEncoding from 'dwebx-encoding'
...
const logger = baseLogger.child({category: 'dweb', subcategory: 'drives'})

// dbs
import * as archivesDb from '../dbs/archives'
import * as hyperDnsDb from '../dbs/dwebx-dns'
...
export async function pullLatestDriveMeta (drive, {updateMTime} = {}) {
  try {
    ...
    logger.info('Updated recorded metadata for ddrive', {key, details})
    return details
  } catch (e) {
...
export async function createNewDrive (manifest = {}) {
  ...
  // write the manifest and default dwebxignore
  await Promise.all([
    drive.pda.writeManifest(manifest)
    // DISABLED drive.pda.writeFile('/.dwebxignore', await settingsDb.get('default_dat_ignore'), 'utf8')
  ])
...
export async function forkDrive (srcDriveUrl, opts = {}) {
  ...
  // copy files
  var ignore = ['/.dwebx', '/.git', '/index.json']
...
/**
 * @desc
 * Get the primary URL for a given dwebx URL
 *
 * @param {string} url
 * @returns {Promise<string>}
 */
export async function getPrimaryUrl (url) {
  ...
  if (!datDnsRecord) return `dweb://${key}/`
  return `dweb://${datDnsRecord.name}/`
}
...
export async function confirmDomain (key) {
  // DISABLED
  // dweb: does not currently use DNS
  // -prf
...
export function fromURLToKey (url, lookupDns = false) {
  ...
  if (urlp.protocol !== 'dweb:' && urlp.protocol !== 'dwebx:') {
    throw new InvalidURLError('URL must be a dweb: or dwebx: scheme')
  }
