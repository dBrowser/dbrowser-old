...
import HyperdriveDaemon from 'ddrive-daemon'
import * as HyperdriveDaemonManager from 'ddrive-daemon/manager'
import { createMetadata } from 'ddrive-daemon/lib/metadata'
import constants from 'ddrive-daemon-client/lib/constants'
import { HyperdriveClient } from 'ddrive-daemon-client'
import datEncoding from 'dwebx-encoding'
import * as pda from 'dbrowser-dwebx-api'
import pm2 from 'pm2'
import EventEmitter from 'events'
import { getEnvVar } from '../lib/env'
import * as logLib from '../logger'
const baseLogger = logLib.get()
const logger = baseLogger.child({category: 'dweb', subcategory: 'daemon'})
...
var client // client object created by ddrive-daemon-client
  ...
  if (isSettingUp) {
    ...
    let interval = setInterval(() => {
      pm2.list((err, processes) => {
        var processExists = !!processes.find(p => p.name === 'ddrive' && p.pm2_env.status === 'online')
...
/**
 * Provides a dbrowser-dwebx-api object for the given drive
 * @param {Object} drive
