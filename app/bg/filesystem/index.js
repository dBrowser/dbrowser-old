...
const logger = logLib.get().child({category: 'dweb', subcategory: 'filesystem'})
import dweb from '../dweb/index'
...
export function isRootUrl (url) {
  return url === browsingProfile.url || url === 'dweb://system/'
}
...
export async function setup () {
  ...
  if (!browsingProfile.url || (typeof browsingProfile.url === 'string' && browsingProfile.url.startsWith('dwebx:'))) {
    let drive = await dweb.drives.createNewRootDrive()
  ...
  // load root drive
  dweb.dns.setLocal('system', browsingProfile.url)
  rootDrive = await dweb.drives.getOrLoadDrive(browsingProfile.url, {persistSession: true})
  ...
  try {
    ...
    if (isInitialCreation) {
      await bookmarks.add({href: 'dbrowser://explorer/', title: 'Explore Files', pinned: true})
      await bookmarks.add({href: 'dbrowser://webterm/', title: 'Terminal', pinned: true})
      await bookmarks.add({href: 'https://userlist.dbrowser.com/', title: 'User Directory', pinned: true})
      ...
      await bookmarks.add({href: 'https://docs.dbrowser.com/', title: 'DBrowserX Documentation', pinned: true})
      await bookmarks.add({href: 'https://dbrowserx.dev/', title: 'DBrowserX Developer Portal', pinned: true})
    }
  } catch (e) {
  ...
  if (profileObj) {
    ...
    profileDriveUrl = `dweb://${profileObj.key}/`
  }
  try {
  ...
  dweb.drives.ensureHosting(hostKeys)
}
...
export function listDrives ({includeSystem} = {includeSystem: false}) {
  var d = drives.slice()
  if (includeSystem) {
    d.unshift({key: rootDrive.url.slice('dweb://'.length)})
  }
  return d
}
...
export async function configDrive (url, {forkOf} = {forkOf: undefined}) {
  ...
  try {
    var key = await dweb.drives.fromURLToKey(url, true)
    var driveCfg = drives.find(d => d.key === key)
    if (!driveCfg) {
      let drive = await dweb.drives.getOrLoadDrive(url)
      ...
      if (manifest.forkOf && typeof manifest.forkOf === 'string') {
        ...
        driveCfg.forkOf.key = await dweb.drives.fromURLToKey(manifest.forkOf, true)
...
export async function setupDefaultProfile ({title, description, thumbBase64, thumbExt}) {
  var drive = await dweb.drives.createNewDrive({title, description})
  if (thumbBase64) {
