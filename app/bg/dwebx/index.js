...
export function getStoragePathFor (key) {
  ...
  tmpdirs[key] = join(tmpdir(), 'dwebx', key)
  return tmpdirs[key]
}
...
export async function convertDatArchive (win, key) {
  ...
  var storagePath = getStoragePathFor(key)
  var drive = await dweb.drives.createNewDrive()
  ...
  var prompt = await prompts.create(win.webContents, 'progress', {label: 'Converting dwebx...'})
  try {
  ...
  await drive.pda.rename('/dwebx.json', drive.session.drive, '/index.json').catch(e => undefined)
  await filesystem.configDrive(drive.url)
  return drive.url
}

async function runConvertProcess (...args) {
  var fullModulePath = join(__dirname, 'bg', 'dwebx', 'converter', 'index.js')
  const opts = {
    stdio: 'inherit',
