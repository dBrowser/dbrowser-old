...
import pda from 'dbrowser-dwebx-api'
import pick from 'lodash.pick'
import _get from 'lodash.get'
import _flattenDeep from 'lodash.flattendeep'
import * as modals from '../../ui/subwindows/modals'
import * as permissions from '../../ui/permissions'
import * as hyperDns from '../../dweb/dns'
import * as capabilities from '../../dweb/capabilities'
import * as drives from '../../dweb/drives'
...
const isSenderBeaker = (sender) => /^(dbrowser:|https?:\/\/(.*\.)?ddrive\.network(:|\/))/.test(sender.getURL())
...
export default {
  async createDrive ({title, description, author, visibility, prompt} = {}) {
    ...
    // only allow these vars to be set by dbrowserx, for now
  ...
  async configure (url, settings, opts) {
    return auditLog.record(this.sender.getURL(), 'configure', {url, ...settings}, undefined, () => (
      timer(to(opts), async (checkin, pause, resume) => {
        ...
        // also, only allow dbrowserx to set 'visibility' for now
        ...
        // only allow dbrowserx to set these manifest updates for now
        if (!isSenderBeaker(this.sender)) {
  ...
  async beakerDiff (srcUrl, dstUrl, opts) {
    ...
    if (!srcUrl || typeof srcUrl !== 'string') {
      throw new InvalidURLError('The first parameter of diff() must be a ddrive URL')
    }
    if (!dstUrl || typeof dstUrl !== 'string') {
      throw new InvalidURLError('The second parameter of diff() must be a ddrive URL')
    }
  ...
  async beakerMerge (srcUrl, dstUrl, opts) {
    assertBeakerOnly(this.sender)
    if (!srcUrl || typeof srcUrl !== 'string') {
      throw new InvalidURLError('The first parameter of merge() must be a ddrive URL')
    }
    if (!dstUrl || typeof dstUrl !== 'string') {
      throw new InvalidURLError('The second parameter of merge() must be a ddrive URL')
    }
    var [src, dst] = await Promise.all([lookupDrive(this.sender, srcUrl), lookupDrive(this.sender, dstUrl)])
...
async function assertReadPermission (drive, sender) {
  ...
  if (typeof drive === 'string') {
    driveUrl = `dweb://${await drives.fromURLToKey(drive, true)}/`
  } else {
    driveUrl = drive.url
  }
...
// - can get a ddrive hash, or ddrive url
