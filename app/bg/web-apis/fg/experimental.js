...
export const setup = function (rpc) {
  ...
  if (['dbrowser:', 'dweb:'].includes(window.location.protocol)) {
    ...
    const datPeersRPC = rpc.importAPI('experimental-dwebx-peers', experimentalDatPeersManifest, opts)
    ...
    experimental.globalFetch = async function globalFetch (input, init) {
      ...
      try {
        ...
        if (e.message === 'Can only send requests to http or https URLs' && request.url.startsWith('dweb://')) {
          // we can just use `fetch` for dweb:// URLs, because dweb:// does not enforce CORS
