...
import filesystemManifest from '../manifests/internal/dbrowserx-filesystem'
import { EventTarget, Event, fromEventStream } from './event-target'
import { createStat } from './stat'

const isDriveUrlRe = /^(dweb:\/\/)?[^\/]+/i

export function setup (rpc) {
  // create the rpc apis
  const hyperdriveRPC = rpc.importAPI('ddrive', hyperdriveManifest, { timeout: false, errors })
  ...
  function massageUrl (url) {
    ...
    if (location.protocol === 'dweb:') {
      if (!isDriveUrlRe.test(url)) {
        url = joinPath('dweb://' + location.hostname, url)
      }
    ...
    if (!isDriveUrlRe.test(url)) {
      // whoops not a valid dweb:// url
      throw new Error('Invalid URL: must be a dweb:// URL')
    }
    return url
  }
  ...
  function createScopedAPI (url) {
    ...
    return {
      ...
      checkout (version) {
        ...
        return createScopedAPI(`dweb://${urlParsed.hostname}${version}/`)
      },
  ...
  var api = {
    ...
    async query (opts) {
      ...
      if (!opts.drive && location.protocol === 'dweb:') {
