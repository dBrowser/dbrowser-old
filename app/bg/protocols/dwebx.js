...
import { getStoragePathFor, downloadDat } from '../dwebx/index'
import { URL } from 'url'
import { promisify } from 'util'
import datDns from '../dwebx/dns'
import datServeResolvePath from '@dbrowser/dwebx-serve-resolve-path'
import ScopedFS from 'scoped-fs'
import * as mime from '../lib/mime'
import * as logLib from '../logger'
const logger = logLib.child({category: 'dwebx', subcategory: 'protocol'})
...
export function register (protocol) {
  protocol.registerStreamProtocol('dwebx', electronHandler)
}
...
export const electronHandler = async function (request, respond) {
  try {
    ...
    var manifest = {}
    try {
      manifest = JSON.parse(await fs.readFile('/dwebx.json'))
    } catch (e) {
      logger.warn('Failed to fetch dwebx:// manifest', {key, error: e})
    }
...
`)})
    ...
    logger.error('Failed to access dwebx', {error: e, url: request.url})
    respond({
      statusCode: 400,
      headers: {'Content-Type': 'text/html'},
      data: intoStream(`<h1>Failed to load DWebX</h1><pre>${e.toString()}</pre>`)
    })
  }
}
