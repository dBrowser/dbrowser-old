...
const logger = logLib.child({category: 'dweb', subcategory: 'dweb-scheme'})
...
export function register (protocol) {
  protocol.registerStreamProtocol('dweb', protocolHandler)
}
...
export const protocolHandler = async function (request, respond) {
  ...
  if (urlp.host.endsWith('.cap')) {
    ...
    if (!cap) {
      return respondError(404, 'No record found for ' + urlp.host, {
        ...
        errorInfo: `No record found for dweb://${urlp.host}`
      })
    }
  ...
  if (filesystem.isRootUrl(`dweb://${driveKey}/`)) {
    // HACK
  ...
  auditLog.record('-browser', 'serve', {url: urlp.origin, path: urlp.pathname}, undefined, async () => {
    ...
    if (entry && entry.isDirectory()) {
      ...
      if (!hasTrailingSlash) {
        ...
        return respondRedirect(`dweb://${urlp.host}${urlp.version ? ('+' + urlp.version) : ''}${urlp.pathname || ''}/${urlp.search || ''}`)
      }
