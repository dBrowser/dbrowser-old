...
export async function checkLabsPerm ({perm, labApi, apiDocsUrl, sender}) {
  ...
  if (urlp.protocol === 'dweb:') {
    // resolve name
    let key = await dweb.dns.resolveName(urlp.hostname)

    // check index.json for opt-in
    let isOptedIn = false
    let drive = dweb.drives.getDrive(key)
    if (drive) {
      let {checkoutFS} = await dweb.drives.getDriveCheckout(drive, urlp.version)
