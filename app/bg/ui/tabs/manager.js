...
export async function setup () {
  ...
  dweb.daemon.on('daemon-restored', () => emitReplaceStateAllWindows())
  dweb.daemon.on('daemon-stopped', () => emitReplaceStateAllWindows())
  ...
  dweb.drives.on('updated', ({details}) => {
    iterateTabs(tab => {
...
export function emitReplaceState (win) {
  ...
  var state = {
    ...
    isDaemonActive: dweb.daemon.isActive()
  }
...
rpc.exportAPI('background-process-views', viewsRPCManifest, {
  ...
  async getNetworkState (tab, opts) {
    ...
    if (tab && tab.driveInfo) {
      let peers = await dweb.daemon.getPeerCount(Buffer.from(tab.driveInfo.key, 'hex'))
      let peerAddresses = undefined
      if (opts && opts.includeAddresses) {
        peerAddresses = await dweb.daemon.listPeerAddresses(tab.driveInfo.discoveryKey)
      }
