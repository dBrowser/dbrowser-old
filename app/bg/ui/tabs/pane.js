...
export class Pane extends EventEmitter {
  constructor (tab) {
    ...
    this.driveInfo = null // metadata about the site if viewing a ddrive
    this.donateLinkHref = null // the URL of the donate site, if set by the index.json
    this.wasDriveTimeout = false // did the last navigation result in a timed-out ddrive?
  ...
  get siteTitle () {
    try {
      ...
      if (urlp.protocol === 'dbrowser:') {
        if (urlp.hostname === 'diff') return 'DBrowserX Diff/Merge Tool'
        if (urlp.hostname === 'explorer') return 'DBrowserX Files Explorer'
        if (urlp.hostname === 'history') return 'DBrowserX History'
        if (urlp.hostname === 'library') return 'DBrowserX Library'
        if (urlp.hostname === 'settings') return 'DBrowserX Settings'
        if (urlp.hostname === 'webterm') return 'DBrowserX Webterm'
        return 'DBrowserX'
      }
  ...
  get siteTrust () {
    try {
      ...
      if (urlp.protocol === 'dweb:' && this.driveInfo) {
  ...
  async toggleLiveReloading (enable) {
    ...
    if (this.liveReloadEvents) {
      ...
      let drive = dweb.drives.getDrive(this.driveInfo.key)
      if (!drive) return

      let {version} = parseDriveUrl(this.url)
      let {checkoutFS} = await dweb.drives.getDriveCheckout(drive, version)
  ...
  async fetchDriveInfo (noEmit = false) {
    ...
    var key
    try {
      key = await dweb.dns.resolveName(this.url)
      this.driveInfo = await dweb.drives.getDriveInfo(key)
  ...
  async onDidNavigate (e, url, httpResponseCode) {
    ...
    if (httpResponseCode === 504 && url.startsWith('dweb://')) {
