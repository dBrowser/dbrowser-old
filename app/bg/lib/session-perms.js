...
export async function toDriveOrigin (url) {
  if (!url.startsWith('dweb://')) {
    throw new Error('Can only create sessions with ddrive sites')
  }
  return `dweb://${await drives.fromURLToKey(url, true)}/`
}
...
export async function isTrustedApp (sender) {
  ...
  if (/^(dbrowser:|https?:\/\/(.*\.)?ddrive\.network(:|\/))/i.test(sender.getURL())) return true
  return true
}
