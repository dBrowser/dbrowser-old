...
import { convertDatArchive } from './dwebx/index'
...
import * as hyperDaemon from './dweb/daemon'
...
const DOT_DESKTOP_FILENAME = 'appimagekit-dbrowser-x.desktop'
...
export async function getDefaultProtocolSettings () {
  if (IS_LINUX) {
    ...
    let [httpHandler, hyperHandler, datHandler] = await Promise.all([
      ...
      exec('xdg-mime query default "x-scheme-handler/dwebx"').catch(err => '')
    ])
    ...
    return {
      ...
      dweb: (hyperHandler || '').toString().trim() === DOT_DESKTOP_FILENAME,
      dwebx: (datHandler || '').toString().trim() === DOT_DESKTOP_FILENAME
    }
  }

  return Promise.resolve(['http', 'dweb', 'dwebx'].reduce((res, x) => {
...
export function restartBrowser () {
  if (updaterState == UPDATER_STATUS_DOWNLOADED) {
    ...
    logger.info('Restarting DBrowserX by restartBrowser()')
    // do a simple restart
    app.relaunch()
