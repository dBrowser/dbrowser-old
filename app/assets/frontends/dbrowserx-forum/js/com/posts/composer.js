...
export class PostComposer extends LitElement {
  ...
  async load () {
    if (location.search && location.search.includes('from-cli')) {
      ...
      if (params.get('url')) {
        ...
        let drive = ddrive.load(urlp.hostname)
        let base64buf = await drive.readFile(urlp.pathname, 'base64')
        this.file = {source: 'ddrive', name: urlp.pathname.split('/').pop(), base64buf}
      }
  ...
  async readUrlMetadata () {
    ...
    if (urlp.protocol === 'dweb:') {
      if (urlp.pathname === '/') {
        try {
          let info = await ddrive.load(urlp.hostname).getInfo({timeout: 10e3})
          this.linkMetadata = {
            success: true,
            driveType: info.type
          }
      ...
      <div class="form-group">
        ...
        <div class="file-input ${success ? 'success' : ''}">
          ...
          <div>
            ...
            <a href="#" @click=${this.onClickSelectHyperdriveFile}>your ddrive</a>
            or
  ...
  async onClickSelectHyperdriveFile (e) {
    ...
    var base64buf = await ddrive.load(sels[0].origin).readFile(sels[0].path, 'base64')
    this.file = {source: 'ddrive', name: sels[0].path.split('/').pop(), base64buf}
    this.runValidation()
  }
...
customElements.define('dbrowserx-post-composer', PostComposer)
