...
export class About extends LitElement {
  ...
  async load () {
    var drive = ddrive.self
    this.info = await drive.getInfo()
    this.requestUpdate()
    this.groupware = await drive.readFile('/dbrowserx-forum/groupware.json').then(JSON.parse).catch(e => undefined)
    this.requestUpdate()
    this.userCount = await uwg.users.count()
    this.requestUpdate()
    this.sidebarMd = await drive.readFile('/dbrowserx-forum/sidebar.md').catch(e => undefined)
    this.requestUpdate()
  }
...
customElements.define('dbrowserx-about', About)
