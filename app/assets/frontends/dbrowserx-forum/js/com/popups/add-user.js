...
export class AddUserPopup extends BasePopup {
  ...
  async validateUserDrive (url) {
    ...
    if (urlp.protocol !== 'dweb:') {
      return {success: false, message: `You must provide a "dweb:" URL. This is "${urlp.protocol}".`}
    }

    var drive = ddrive.load(url)
    var info
    try {
  ...
  static destroy () {
    return BasePopup.destroy('dbrowserx-add-user-popup')
  }
      ...
      <form @submit=${this.onSubmit}>
        ...
        ` : html`
          <section>
            ...
            <input autofocus name="userUrl" tabindex="2" value=${this.userUrl || ''} placeholder="dweb://" @change=${this.onChangeUserUrl} class=${this.errors.userUrl ? 'has-error' : ''} />
...
customElements.define('dbrowserx-add-user-popup', AddUserPopup)