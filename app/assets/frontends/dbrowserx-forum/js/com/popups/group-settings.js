...
export class GroupSettingsPopup extends BasePopup {
  ...
  static get styles () {
    ...
    .img-ctrl dbrowserx-img-fallbacks {
      height: 90px;
    }
  ...
  async load () {
    ...
    var sidebarMd = await self.readFile('/dbrowserx-forum/sidebar.md').catch(e => '')
    var pinnedMessageMd = await self.readFile('/dbrowserx-forum/pinned-message.md').catch(e => '')
  ...
  static destroy () {
    return BasePopup.destroy('dbrowserx-edit-profile-popup')
  }
  ...
  async onSubmit (e) {
    ...
    try {
      let drive = ddrive.self
      await drive.configure({
        title: this.title,
        description: this.description
      })
      if (this.sidebarMd) {
        await drive.mkdir('/dbrowserx-forum').catch(e => undefined)
        await drive.writeFile('/dbrowserx-forum/sidebar.md', this.sidebarMd)
      } else {
        await drive.unlink('/dbrowserx-forum/sidebar.md').catch(e => undefined)
      }
      if (this.pinnedMessageMd) {
        await drive.mkdir('/dbrowserx-forum').catch(e => undefined)
        await drive.writeFile('/dbrowserx-forum/pinned-message.md', this.pinnedMessageMd)
      } else {
        await drive.unlink('/dbrowserx-forum/pinned-message.md').catch(e => undefined)
      }
...
customElements.define('dbrowserx-group-settings-popup', GroupSettingsPopup)
