...
var groupDrive = ddrive.self
var profileCache = {}
export const profiles = {
  async setUser (url) {
    userDrive = ddrive.load(url)
...
export const posts = {
  ...
  async list (
    ...
    var drive = ddrive.load(author || location)
  ...
  async addLink ({href, title, driveType}, drive = undefined) {
    ...
    var path = `/dbrowserx-forum/posts/${Date.now()}.goto`
  ...
  async addTextPost ({title, content}, drive = undefined) {
    ...
    if (!isNonemptyString(title)) throw new Error('Title is required')
    drive = drive || userDrive
    var path = `/dbrowserx-forum/posts/${Date.now()}.md`
    await ensureParentDir(path, drive)
  ...
  async addFile ({title, ext, base64buf}, drive = undefined) {
    ...
    drive = drive || userDrive
    var path = `/dbrowserx-forum/posts/${Date.now()}.${ext}`
    await ensureParentDir(path, drive)
  ...
  async changeTitle (post, newTitle) {
    ...
    var path = `/dbrowserx-forum/posts/${filename}`
...
export const comments = {
  ...
  async count ({author, href, sort, reverse} = {author: undefined, href: undefined, sort: undefined, reverse: undefined}) {
    href = href ? normalizeUrl(href) : undefined
    let drive = ddrive.load(author || location)
  ...
  async add ({href, parent, content}, drive = undefined) {
    ...
    var path = `/dbrowserx-forum/comments/${Date.now()}.md`
    drive = drive || userDrive
    await ensureParentDir(path, drive)
  ...
  async update (comment, {content}) {
    if (!isNonemptyString(content)) throw new Error('Content is required')
    var commentPath = `/dbrowserx-forum/comments/${comment.path.split('/').pop()}`
...
export const votes = {
  ...
  async put (href, vote, drive = undefined) {
    ...
    var path = `/dbrowserx-forum/votes/${Date.now()}.goto`
    await ensureParentDir(path, drive)
...
function getPostsPaths (author) {
  if (author) {
    return `/dbrowserx-forum/posts/*`
  } else {
    return `/users/*/dbrowserx-forum/posts/*`
  }
}
...
function getCommentsPaths (author) {
  if (author) {
    return `/dbrowserx-forum/comments/*.md`
  } else {
    return `/users/*/dbrowserx-forum/comments/*.md`
  }
}
...
function getVotesPaths (author) {
  if (author) {
    return `/dbrowserx-forum/votes/*.goto`
  } else {
    return `/users/*/dbrowserx-forum/votes/*.goto`
  }
}
