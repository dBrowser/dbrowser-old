...
export class App extends LitElement {
  ...
  async doCommentRedirect () {
    try {
      var comment = await uwg.comments.get(this.routeParams.groups.id, `/dbrowserx-forum/comments/${this.routeParams.groups.filename}`)
    ...
    return html`
      ...
      <header>
        ...
        ${this.session?.user?.group?.isMember ? html`
          ...
          <dbrowserx-search-input placeholder="Search this group"></dbrowserx-search-input>
          <a
  ...
  renderView () {
    switch (this.route) {
      case 'home': return html`
        <dbrowserx-posts-view loadable .user=${this.session?.user}></dbrowserx-posts-view>
      `
      case 'compose': return html`
        <dbrowserx-compose-view loadable .user=${this.session?.user}></dbrowserx-compose-view>
      `
      case 'comments': return html`
        <dbrowserx-comments-view loadable .user=${this.session?.user}></dbrowserx-comments-view>
      `
      case 'users': return html`
        <dbrowserx-users-view loadable .user=${this.session?.user}></dbrowserx-users-view>
      `
      case 'notifications': return html`
        <dbrowserx-notifications-view loadable .user=${this.session?.user}></dbrowserx-notifications-view>
      `
      case 'search': return html`
        <dbrowserx-search-view loadable .user=${this.session?.user}></dbrowserx-search-view>
      `
      case 'userProfile':
      case 'userPosts': return html`
        <dbrowserx-profile-view loadable .user=${this.session?.user} profile-id=${this.routeParams.groups.id} ?admin-ctrls=${this.groupInfo.writable}></dbrowserx-profile-view>
      `
      case 'userComments': return html`
        <dbrowserx-profile-view loadable .user=${this.session?.user} profile-id=${this.routeParams.groups.id} subview="comments" ?admin-ctrls=${this.groupInfo.writable}></dbrowserx-profile-view>
      `
