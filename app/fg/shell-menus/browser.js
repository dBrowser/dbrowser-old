...
class BrowserMenu extends LitElement {
      ...
      autoUpdaterEl = html`
        <div class="section auto-updater">
          <div class="menu-item auto-updater" @click=${this.onClickRestart}>
            ...
            <span class="label">Restart to update DBrowserX</span>
          </div>
        </div>
      `
    }
  ...
  async onNewHyperdrive () {
    bg.shellMenus.close()
    const url = await bg.ddrive.createDrive()
  ...
  async onNewHyperdriveFromFolder (e) {
    ...
    var url = await bg.ddrive.createDrive({
      title: folder[0].split('/').pop(),
      prompt: false
    })
    await bg.ddrive.importFromFilesystem({src: folder[0], dst: url})
