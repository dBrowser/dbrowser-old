    ...
    if (params.url.startsWith('dweb://')) {
      ...
      try {
        ...
        while (pathParts.length > 0) {
          let st = await bg.ddrive.stat(joinPath(urlp.origin, pathParts.join('/')))
          if (st.mount) {
            driveInfo = await bg.ddrive.getInfo(st.mount.key)
            break
          }
          pathAcc.unshift(pathParts.pop())
        }
        if (!driveInfo) {
          driveInfo = await bg.ddrive.getInfo(urlp.origin)
        }

        // make sure it can be shared
        if (driveInfo && driveInfo.url !== 'dweb://system/') {
