...
class ShellWindowToolbarMenu extends LitElement {
  ...
  render () {
    ...
    return html`
      ...
      <a data-href="https://userlist.dbrowser.com/" title="DBrowserX User Directory" @mousedown=${this.onMousedownLink}>User Directory</a>
      <a data-href="https://dbrowserx.dev/" title="Developer Portal" @mousedown=${this.onMousedownLink}>Dev Portal</a>
  ...
  async onMousedownBookmark (e, index) {
    ...
    if (e.button === 2 || (e.button === 1 && (e.metaKey || e.ctrlKey))) {
      ...
      menuChoice = await bg.dBrowserX.showContextMenu(menu)
    } 
    if (menuChoice === 'tab' || e.button === 1) {
      ...
      bg.dBrowserX.newWindow({pages: [item.href]})
  ...
  async onMainContextmenu (e) {
    ...
    var x = e.clientX
    var y = e.clientY
    var menuChoice = await bg.dBrowserX.showContextMenu([
