...
class NavbarLocation extends LitElement {
  ...
  constructor () {
    ...
    setInterval(async () => {
      if (!this.url.startsWith('dweb://')) return
  ...
  get isHyperdrive () {
    return this.url.startsWith('dweb://')
  }
  ...
  renderInputPretty () {
    ...
    if (/^(dweb|http|https|dbrowserx|dwebx):\/\//.test(this.url)) {
      try {
        ...
        // if (['dwebx:'].includes(protocol)) cls += ' protocol-trusted'
        // if (['dbrowser:'].includes(protocol)) cls += ' protocol-trusted'
        return html`
          <div class="input-pretty" @mouseup=${this.onClickLocation}>
  ...
  renderDatConverterBtn () {
    if (this.url.startsWith('dwebx:')) {
      return html`
        <button class="dwebx-converter" title="Convert to DDrive" @click=${this.onClickConvertDat}>
...
button.dwebx-converter {
  width: auto;
  font-size: 13px;
  line-height: 23px;
  background: var(--bg-color--location-dwebx-convert-btn);
  border-radius: 5px;
  margin: 2px;
  padding: 0 9px;
  font-weight: 500;
  color: var(--text-color--location-dwebx-convert-btn);
}

button.dwebx-converter:hover {
  cursor: pointer;
  background: var(--bg-color--location-dwebx-convert-btn--hover);
}
