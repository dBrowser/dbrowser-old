...
class SelectFileModal extends LitElement {
  ...
  async init (params, cbs) {
    ...
    this.driveInfo = !this.isVirtualListing ? await bg.ddrive.getInfo(this.drive) : undefined
    await this.readdir()
    ...
    this.drives.push({url: 'dweb://system/', info: {title: 'System Drive', writable: true}})
    this.drives.sort((a, b) => (a.info.title || '').toLowerCase().localeCompare(b.info.title || ''))
    this.contacts = await bg.contacts.list()
    this.contacts.push(await bg.ddrive.getInfo((await bg.dBrowserX.getProfile()).key))
  ...
  async readdir () {
    ...
    var files = await bg.ddrive.readdir(joinPath(this.drive, this.path), {includeStats: true})
    files.forEach(file => {
  ...
  async onSubmit (e) {
    ...
    if (this.isVirtualListing) {
      ...
      if (this.drive.startsWith('dweb://')) {
        this.driveInfo = await bg.ddrive.getInfo(this.drive)
      }
      return this.goto('/')
    }
