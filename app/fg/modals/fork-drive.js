...
class ForkDriveModal extends LitElement {
  ...
  async init (params, cbs) {
    ...
    // fetch drive info
    this.driveInfo = await bg.ddrive.getInfo(this.base.url)
  ...
  render () {
    ...
    switch (this.state) {
      case STATES.READY:
        progressEl = html`<div class="fork-dwebx-progress">Ready to ${this.isDetached ? 'make a copy' : 'fork'}.</div>`
        actionBtn = html`<button type="submit" class="btn primary" tabindex="5">${this.isDetached ? 'Copy drive' : 'Create fork'}</button>`
        break
      case STATES.DOWNLOADING:
        progressEl = html`<div class="fork-dwebx-progress">Downloading remaining files...</div>`
        actionBtn = html`<button type="submit" class="btn" disabled tabindex="5"><span class="spinner"></span></button>`
        break
      case STATES.CLONING:
        progressEl = html`<div class="fork-dwebx-progress">Downloading and copying...</div>`
        actionBtn = html`<button type="submit" class="btn" disabled tabindex="5"><span class="spinner"></span></button>`
        break
    }
  ...
  async onSubmit (e) {
    ...
    // await bg.ddrive.download(this.base.url)

    this.state = STATES.CLONING
    try {
      var url = await bg.ddrive.forkDrive(this.base.url, {
