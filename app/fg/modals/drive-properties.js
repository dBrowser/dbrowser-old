...
class DrivePropertiesModal extends LitElement {
  ...
  async onSubmit (e) {
    ...
    if (thumbInput.files[0]) {
      ...
      await Promise.all([
        bg.ddrive.unlink(joinPath(this.url, '/thumb.png')).catch(e => null),
        bg.ddrive.unlink(joinPath(this.url, '/thumb.jpg')).catch(e => null),
        bg.ddrive.unlink(joinPath(this.url, '/thumb.jpeg')).catch(e => null)
      ])
      await bg.ddrive.writeFile(joinPath(this.url, `/thumb.${ext}`), await bufPromise)
    }

    // handle props
    await bg.ddrive.configure(this.url, newProps).catch(e => null)
