...
const bg = {
  dBrowserX: rpc.importAPI('dbrowser-x', browserManifest),
  bookmarks: rpc.importAPI('bookmarks', bookmarksManifest),
  ddrive: rpc.importAPI('ddrive', hyperdriveManifest),
  ...
  beakerFs: rpc.importAPI('dbrowserx-filesystem', beakerFsManifest),
...
class LocationBar extends LitElement {
  ...
  reset () {
    ...
    this.searchEnginesPromise = bg.dBrowserX.getSetting('search_engines')
  }
      ...
      <div class="wrapper">
        ...
        <div class="search-engines">
          ...
          <div class="list">
            ...
            ${''/* TODO restore at some point esearchLink('DBrowserX', `dbrowser://search/?q=${encodeURIComponent(this.inputQuery)}`) */}
          </div>
        </div>
      </div>
    `
...
async function searchDrive (urlp) {
  try {
    ...
    var items = await bg.ddrive.readdir(joinPath(urlp.origin, parentFolderPath), {timeout: 2e3, includeStats: true})
    if (nameFilter) {
