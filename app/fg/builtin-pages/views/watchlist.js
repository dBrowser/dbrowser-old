/* globals DatArchive dbrowserx confirm localStorage */
...
let watchlist = []
let selectedItems = []
let wlEvents = dbrowserx.watchlist.createEventsStream()
...
async function loadWatchlist () {
  watchlist = await dbrowserx.watchlist.list()
  sortWatchlist()
}
...
async function addToWatchlist (url, description) {
  ...
  try {
    await dbrowserx.watchlist.add(url, {description, seedWhenResolved: false})
  ...
  return yo`
    ...
    >
      ...
      <span class="watchlist-item-title">
        <img class="favicon" src="dbrowserx-favicon:32,${row.url}" />
        ...
        <div class="builtin-main">
          ...
          <div>
            ...
            ${resolved.length === 0 && unresolved.length === 0
              ? yo`
                <div class="view empty">
                  <i class="fa fa-eye"></i>
                  <p>
                    You aren${"'"}t watching for any dwebx sites!
                  </p>
                </div>`
              : ''
            }
            ...
            <p class="builtin-hint">
              ...
              Your Watchlist contains websites you${"'"}ve asked DBrowserX to find for you.
...
async function onDelete (e, item) {
  ...
  try {
    await dbrowserx.watchlist.remove(item.url)
  } catch (e) {
    console.error(e)
...
async function onDeleteSelected () {
  await Promise.all(selectedItems.map(async a => {
    a.checked = false
    try {
      await dbrowserx.watchlist.remove(a.url)
    } catch (e) {
      toast.create(`Could not remove site from watchlist`, 'error')
    }
  }))
