...
async function setup () {
  ...
  if (archiveKey) {
    ...
    updatePeers()
    dbrowserx.archives.addEventListener('network-changed', ({details}) => {
  ...
  var debugEvents = dbrowserx.archives.createDebugStream()
  debugEvents.addEventListener(archiveKey || 'all', onLog)
  logEntries = (await dbrowserx.archives.getDebugLog(archiveKey)).split('\n').map(parseJSON).filter(Boolean)
...
async function parseURL () {
  ...
  try {
    ...
    if (path.startsWith('dwebx://')) {
      path = path.slice('dwebx://'.length)
    }
    var parts = /^([^/]+)/.exec(path)
    var key = parts[1]
    ...
    throw new Error('Invalid dwebx URL')
  }
}
...
function renderUsage (err) {
  yo.update(document.querySelector('main'), yo`
    <main>
      <h1>${err}</h1>
      <h2>A valid dwebx URL is required in the URL path.</h2>
    </main>
  `)
}
