...
import {Archive} from 'dbrowser-bpages-lib'
...
async function setup () {
  ...
  try {
    ...
    if (e.name === 'TimeoutError') {
      loadError = 'The dweb site could not be found on the network. Make sure it is online and that you are connected to the Internet.'
    } else {
...
async function readWorkingDatJson () {
  var dwebJson
  try {
    dwebJson = JSON.parse(await workingCheckout.readFile('/dweb.json'))
  } catch (e) {
    dwebJson = {}
  }
  dwebJson.title = dwebJson.title || ''
  dwebJson.description = dwebJson.description || ''
  return dwebJson
}
