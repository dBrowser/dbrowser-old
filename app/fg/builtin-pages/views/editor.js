/* globals DatArchive dbrowserx monaco editor diffEditor localStorage hljs confirm sessionStorage location alert history */
...
async function setup () {
  ...
  let browserInfo = dbrowserx.browser.getInfo()
  ...
  try {
    ...
    if (e.name === 'TimeoutError') {
      loadError = 'The dwebx site could not be found on the network. Make sure it is online and that you are connected to the Internet.'
    } else {
...
async function localCompare () {
  ...
  try {
    currentDiff = await dbrowserx.archives.diffLocalSyncPathListing(archive.url, {compareContent: true, shallow: true})
...
async function readWorkingDatJson () {
  var datJson
  try {
    datJson = JSON.parse(await workingCheckout.readFile('/dwebx.json'))
  } catch (e) {
    datJson = {}
  }
...
async function onSetFavicon (e) {
  ...
  if (imageData) {
    ...
    await dbrowserx.sitedata.set(archive.url, 'favicon', '') // clear cache
  }
  if (!models.getActive()) {
...
async function onArchiveSave (e) {
  await dbrowserx.archives.add(archive.url)
  location.reload()
}

async function onArchiveUnsave (e) {
  await dbrowserx.archives.remove(archive.url)
  location.reload()
}
...
async function onArchiveDeletePermanently (e) {
  ...
  try {
    await dbrowserx.archives.delete(archive.url)
...
async function onImportFiles (e) {
  ...
  var files = await dbrowserx.browser.showOpenDialog({
    title: 'Import files',
    buttonLabel: 'Import',
...
async function onImportFolder (e) {
  ...
  var dst = workingCheckout.url + e.detail.path
  var folders = await dbrowserx.browser.showOpenDialog({
...
async function onCommitFile (e) {
  await op('Committing...', async () => {
    ...
    await dbrowserx.archives.publishLocalSyncPathListing(archive.url, {paths: [path]})
    models.exitDiff()
...
async function onRevertFile (e) {
  await op('Reverting...', async () => {
    const path = e.detail.path
    await dbrowserx.archives.revertLocalSyncPathListing(archive.url, {paths: [path]})
...
async function onCommitAll (e) {
  await op('Committing...', async () => {
    ...
    await dbrowserx.archives.publishLocalSyncPathListing(archive.url, {shallow: false, paths})
...
async function onRevertAll (e) {
  await op('Reverting...', async () => {
    // revert
    var paths = fileDiffsToPaths(currentDiff)
    await dbrowserx.archives.revertLocalSyncPathListing(archive.url, {shallow: false, paths})
...
async function onTogglePreviewMode () {
  ...
  try {
    ...
    await dbrowserx.archives.setUserSettings(archive.url, {previewMode})
...
async function onChangeSyncPath () {
  ...
  var defaultPath = ''
  if (!currentPath) {
    let basePath = await dbrowserx.browser.getSetting('workspace_default_path')
    defaultPath = await dbrowserx.browser.getDefaultLocalPath(basePath, archive.info.title)
  }
  ...
  try {
    ...
    await dbrowserx.archives.setUserSettings(archive.url, {previewMode: true})

    // set folder
    await dbrowserx.archives.setLocalSyncPath(archive.url, localSyncPath)

    // open folder and reload page
    dbrowserx.browser.openFolder(localSyncPath)
...
async function onRemoveSyncPath (e) {
  ...
  try {
    await dbrowserx.archives.setLocalSyncPath(archive.url, null)
    window.location.reload()
  } catch (e) {
    toast.create(e.toString(), 'error', 5e3)
    console.error(e)
