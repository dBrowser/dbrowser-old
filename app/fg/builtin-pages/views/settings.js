...
import DatCache from '../com/settings/dwebx-cache'
...
async function setup () {
  ...
  // wire up events
  browserEvents = dbrowserx.browser.createEventsStream()
  ...
  // fetch data
  browserInfo = dbrowserx.browser.getInfo()
  settings = await dbrowserx.browser.getSettings()
  defaultProtocolSettings = await dbrowserx.browser.getDefaultProtocolSettings()
  users = await dbrowserx.users.list()
...
function renderSidebar () {
  return yo`
    <div class="builtin-sidebar">
      ...
      <div class="nav-item ${activeView === 'dwebx-network' ? 'active' : ''}" onclick=${() => onUpdateView('dwebx-network')}>
        <i class="fa fa-angle-right"></i>
        DWebX Network
      </div>
      ...
      <div class="nav-item ${activeView === 'dwebx-cache' ? 'active' : ''}" onclick=${() => onUpdateView('dwebx-cache')}>
        <i class="fa fa-angle-right"></i>
        DWebX Cache
      </div>
...
function renderUsers () {
  const onEdit = async (e, user) => {
    ...
    var opts = await dbrowserx.browser.showModal('user', user)
  ...
  const onDelete = async (e, user) => {
    ...
    if (confirm('Are you sure?')) {
      await dbrowserx.users.remove(user.url)
      location.reload()
    }
  }
...
function renderDatSettings () {
  function onChangeUpload (e) {
    ...
    dbrowserx.browser.setSetting('dat_bandwidth_limit_up', settings.dat_bandwidth_limit_up)
    renderToPage()
  ...
  function onChangeDownload (e) {
    ...
    dbrowserx.browser.setSetting('dat_bandwidth_limit_down', settings.dat_bandwidth_limit_down)
    renderToPage()
  ...
  return yo`
    <div class="section dwebx-bandwidth">
      <h2 id="dwebx-bandwidth" class="subtitle-heading">DWebX Settings</h2>

      <p>
        Set a limit on your bandwidth usage for DWebX.
      </p>

      <div class="inputs">
        <div>
          <label for="dwebx-upload-limit">Upload limit (MB/s)</label>
          <input id="dwebx-upload-limit" type="text" placeholder="Unlimited" value=${up} onchange=${onChangeUpload} />
        </div>
        <div>
          <label for="dwebx-download-limit">Download limit (MB/s)</label>
          <input id="dwebx-download-limit" type="text" placeholder="Unlimited" value=${down} onchange=${onChangeDownload} />
        </div>
      </div>
    </div>
  `
...
function renderDefaultDatIgnoreSettings () {
  return yo`
    <div class="section default-dwebx-ignore">
      <h2 id="default-dwebx-ignore" class="subtitle-heading">Default .dwebxignore</h2>
...
function renderAnalyticsSettings () {
  function toggle () {
    ...
    dbrowserx.browser.setSetting('analytics_enabled', settings.analytics_enabled)
    renderToPage()
  }
...
function renderDatCache () {
  return yo`
    <div class="view">
      <div class="section">
        <h2 id="dwebx-cache" class="subtitle-heading">DWebX cache</h2>
        ${datCache.render()}
      </div>
    </div>
  `
  ...
  return yo`
    <div class="view not-fullwidth">
      ...
      <div class="section">
        ...
        <ul>
          <li><a href="https://dbrowser.com/docs/using-dbrowserx">Take a tour of DBrowserX</a></li>
...
function renderProtocolSettings () {
  function toggleRegistered (protocol) {
    ...
    if (defaultProtocolSettings[protocol]) {
      dbrowserx.browser.setAsDefaultProtocolClient(protocol)
    } else {
      dbrowserx.browser.removeAsDefaultProtocolClient(protocol)
    }
    renderToPage()
  }
...
function renderDefaultToDatSetting () {
  return yo`
    <div class="section default-to-dwebx">
      <h2 id="protocol" class="subtitle-heading">Default redirect to dwebx</h2>
      ...
      <label class="toggle">
        ...
        <div class="switch"></div>
        <span class="text">
          Automatically redirect to dwebx:// when available
        </span>
      </label>
    </div>`
}
...
function onCustomStartPageChange (e) {
  ...
  dbrowserx.browser.setSetting('custom_start_page', settings.custom_start_page)
}

function onClickCheckUpdates () {
  // trigger check
  dbrowserx.browser.checkForUpdates()
}

function onClickCheckPrereleases (e) {
  e.preventDefault()
  dbrowserx.browser.checkForUpdates({prerelease: true})
}

function onToggleAutoRedirect () {
  settings.auto_redirect_to_dat = isAutoRedirectEnabled() ? 0 : 1
  renderToPage()
  dbrowserx.browser.setSetting('auto_redirect_to_dat', settings.auto_redirect_to_dat)
}

function onToggleAutoUpdate () {
  settings.auto_update_enabled = isAutoUpdateEnabled() ? 0 : 1
  renderToPage()
  dbrowserx.browser.setSetting('auto_update_enabled', settings.auto_update_enabled)
}

async function onUpdateWorkspaceDefaultPath () {
  let path = await dbrowserx.browser.showOpenDialog({
  ...
  if (path) {
    ...
    dbrowserx.browser.setSetting('workspace_default_path', settings.workspace_default_path)
    renderToPage()
...
async function onChangeDefaultDatIgnore (e) {
  try {
    await dbrowserx.browser.setSetting('default_dat_ignore', e.target.value)
    toast.create('Default .dwebxignore updated')
  } catch (e) {
    console.error(e)
  }
}
...
function onClickRestart () {
  dbrowserx.browser.restartBrowser()
}
