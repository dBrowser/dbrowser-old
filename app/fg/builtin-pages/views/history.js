...
async function loadVisits (offset, cb) {
  ...
  var rows = await dbrowserx.history.getVisitHistory({
    before: +before,
    after: +after,
    offset,
    limit: BATCH_SIZE,
...
function onClickDelete (i) {
  ...
  var v = visits[i]
  visits.splice(i, 1)
  dbrowserx.history.removeVisit(v.url)
  render()
}
...
function onClickDeleteBulk () {
  ...
  if (period === 'all') {
    visits = []
    dbrowserx.history.removeAllVisits()
    render()
  } else {
    ...
    dbrowserx.history.removeVisitsAfter(threshold)
