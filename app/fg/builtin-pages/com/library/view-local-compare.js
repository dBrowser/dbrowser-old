...
export default class LibraryViewLocalCompare {
  ...
  async loadCompareDiff () {
    // load diff
    if (this.target) {
      this.compareDiff = await dbrowserx.archives.diffLocalSyncPathListing(this.target.url, {compareContent: true, shallow: true})
  ...
  async loadFileDiffs (compareDiff) {
    ...
    const loadNextFileDiff = async () => {
      ...
      // run the diff
      try {
        d.diff = await dbrowserx.archives.diffLocalSyncPathFile(this.target.url, d.path)
  ...
  async onMerge (base, target, opts = {}) {
    ...
    try {
      if (isPublish) {
        await dbrowserx.archives.publishLocalSyncPathListing(this.target.url, opts)
      } else {
        await dbrowserx.archives.revertLocalSyncPathListing(this.target.url, opts)
      }
