/* globals dbrowserx DatArchive */

import yo from 'yo-yo'
import {Archive as LibraryDatArchive} from 'builtin-pages-lib'
import {diffLines} from '@dbrowser/dwebx-archive-file-diff'
...
export default class LibraryViewCompare {
  ...
  async loadCompareDiff () {
    ...
    // TEMP
    // diff the dwebx.json so that we can render it in a grayed-out interface
    // this should be replaced with a semantically-aware dwebx.json diff tool
    // -prf
    // diffLines(target, '/dwebx.json', base, '/dwebx.json').then(diff => {
    //   var d = {diff, path: 'dwebx.json', debug_isManifest: true, debug_shouldIgnoreChange: false}
