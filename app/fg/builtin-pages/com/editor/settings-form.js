...
const DOMAIN_NAME_HELP = yo`<span>
  ...
  <a class="link" href="https://dbrowser.com/docs/guides/use-a-domain-name-with-dwebx" target="_blank">Follow this guide to setup your DNS record</a>,
...
const DONATE_LINK_HELP = yo`<span>
  Enter a link to your donation page and DBrowserX will show a <span class="fa fa-donate"></span> icon in your site's URL bar.
</span>`
      ...
      <div class="settings-form-inner">
        ${section('info', 'Information', yo`
          <div class="controls">
            ...
            ${linkControl(workingDatJson, {isReadonly, label: 'Donation Link', name: 'payment', placeholder: 'e.g. http://opencollective.com/dbrowserx', help: DONATE_LINK_HELP})}
          </div>
        `)}
        ...
        ${isApp ? section('perms', 'Permissions', yo`
          <div class="controls perms-grid">
            ${permissionControl(workingDatJson, {
              isReadonly,
              label: 'Follows API',
              documentation: 'dwebx://unwalled.garden/docs/api/follows',
            ...
            ${permissionControl(workingDatJson, {
              isReadonly,
              label: 'Posts API',
              documentation: 'dwebx://unwalled.garden/docs/api/posts',
            ...
            ${permissionControl(workingDatJson, {
              isReadonly,
              label: 'Bookmarks API',
              documentation: 'dwebx://unwalled.garden/docs/api/bookmarks',
            ...
            ${permissionControl(workingDatJson, {
              isReadonly,
              label: 'Comments API',
              documentation: 'dwebx://unwalled.garden/docs/api/comments',
            ...
            ${permissionControl(workingDatJson, {
              isReadonly,
              label: 'Reactions API',
              documentation: 'dwebx://unwalled.garden/docs/api/reactions',
            ...
            ${permissionControl(workingDatJson, {
              isReadonly,
              label: 'Votes API',
              documentation: 'dwebx://unwalled.garden/docs/api/votes',
...
function domainNameControl (workingCheckout, archiveInfo, manifest, opts) {
  ...
  async function doSave (value) {
    // update dwebx.json
    var m = JSON.parse(await workingCheckout.readFile('/dwebx.json'))
    m.domain = value
    await workingCheckout.writeFile('/dwebx.json', JSON.stringify(m, null, 2))
          ...
          <div class="dns-control">
            <div class="dns-control-msg readonly">
              ${archiveInfo.domain ? yo`
                <a href="dwebx://${archiveInfo.domain}" class="link" target="_blank">${archiveInfo.domain}</a>
        ...
        : manifest.domain
          ? yo`
            <div class="dns-control">
              <div class="dns-control-msg">
                ...
                <a href="dwebx://${manifest.domain}" class="link" target="_blank">${manifest.domain}</a>
                ${isWrong ? 'is not confirmed.' : ''}
                ${isNotCommitted ? '(Make sure to commit the updated dwebx.json.)' : ''}
              </div>
              <div>
          ...
          : possibleDomain
            ? yo`
              <div class="dns-control">
                <div class="dns-control-msg">
                  Use <a href="dwebx://${possibleDomain}" class="link" target="_blank">${possibleDomain}</a>?
                </div>
                <div>
...
async function onSubmitSettings (e, workingCheckout, workingDatJson) {
  ...
  if (canSave) {
    ...
    // write manifest
    await workingCheckout.writeFile('/dwebx.json', JSON.stringify(workingDatJson, null, 2))
...
function onOpenFolder (e, path) {
  e.preventDefault()
  e.stopPropagation()
  dbrowserx.browser.openFolder(path)
}
