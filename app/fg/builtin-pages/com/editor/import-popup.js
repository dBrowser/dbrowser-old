...
async function onSubmit (e) {
  ...
  try {
    ...
    for (let src of srcs) {
      await DatArchive.importFromFilesystem({src, dst: constructedDst, ignore: ['dwebx.json'], inplaceImport: false})
    }
  } catch (e) {
    toast.create(e.toString(), 'error')
    isLoading = false
...
async function doDryRun () {
  try {
    ...
    let constructedDst = dstOrigin + (dst.startsWith('/') ? dst : `/${dst}`)
    for (let src of srcs) {
      changes.push(await DatArchive.importFromFilesystem({src, dst: constructedDst, ignore: ['dwebx.json'], inplaceImport: false, dryRun: true}))
    }
