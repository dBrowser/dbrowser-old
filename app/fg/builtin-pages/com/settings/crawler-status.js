...
export default class CrawlerStatus {
  ...
  async load () {
    if (!this.crawlStates) {
      ...
      let crawlerEvents = dbrowserx.crawler.createEventsStream()
    ...
    this.crawlStates = await dbrowserx.crawler.getCrawlStates()
    this.sort()
    this.rerender()
  }
  ...
  renderStatusRow (row) {
    return yo`
      <div class="ll-row archive" oncontextmenu=${e => this.onContextmenuRow(e, row)}>
        <img class="favicon" src="dbrowserx-favicon:32,${row.url}?cache=${faviconCacheBuster}" />
  ...
  onClickCrawlAll () {
    this.crawlStates.forEach(row => dbrowserx.crawler.crawlSite(row.url))
  ...
  async onClickResetAll () {
    ...
    await Promise.all(this.crawlStates.map(async (row) => {
      await dbrowserx.crawler.resetSite(row.url)
  ...
  async onContextmenuRow (e, row) {
    ...
    const items = [
      ...
      },
      {
        label: 'Crawl site',
        click: () => {
          dbrowserx.crawler.crawlSite(row.url)
          toast.create('Crawl triggered')
        }
      },
      {
        label: 'Rebuild index',
        click: () => {
          dbrowserx.crawler.resetSite(row.url)
          Object.assign(row, initialState(row.url, row.title, null))
          toast.create('Index was deleted and will now rebuild')
          this.rerender()
        }
