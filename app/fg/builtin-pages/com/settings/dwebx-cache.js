/* globals dbrowserx DatArchive confirm */
...
export default class DatCache {
  constructor () {
    ...
    dbrowserx.archives.addEventListener('network-changed', throttle(this.onNetworkChanged.bind(this), 1e3))
  }
  ...
  async fetchArchives () {
    this.archives = await dbrowserx.archives.list({isSaved: false, isOwner: false})
    this.sortArchives()
  ...
  render () {
    if (!this.archives) {
      ...
      return yo`<div id=${'dwebx-cache-' + this.id} class="dwebx-cache loading">Loading...</div>`
    }

    return yo`
      <div id=${'dwebx-cache-' + this.id} class="dwebx-cache">
        <div class="dwebx-cache-actions">
  ...
  rerender () {
    let el = document.querySelector('#dwebx-cache-' + this.id)
  ...
  renderArchive (archive) {
    ...
    return yo`
      <div class="ll-row archive ${highlightedCls}" oncontextmenu=${e => this.onContextmenuArchive(e, archive)}>
        <img class="favicon" src="dbrowserx-favicon:32,${archive.url}" />
  ...
  async onDeleteFiles (archive) {
    try {
      ...
      const res = await dbrowserx.archives.delete(archive.key)
  ...
  async onClearCache ({isOwner} = {isOwner: false}) {
    ...
    try {
      const results = await dbrowserx.archives.clearGarbage({isOwner})
      await dbrowserx.archives.clearDnsCache()
      console.debug('DWebX cache cleared', results)
