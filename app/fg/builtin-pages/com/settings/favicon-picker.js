...
export default function (opts) {
  ...
  if (!builtinFaviconsList) {
    dbrowserx.browser.listBuiltinFavicons()
      .catch(err => {
        loadError = err
...
async function onClickIcon (v) {
  selectedFavicon = v
  onSelect(await dbrowserx.browser.getBuiltinFavicon(selectedFavicon))
  rerender()
}
...
async function onCreateFavicon (e) {
  ...
  let v = await dbrowserx.browser.imageToIco(customFavicon)
  if (v) {
    onSelect(v)
    rerender()
  }
...
async function uploadFavicon () {
  let v = await dbrowserx.browser.uploadFavicon()
  if (v) {
    onSelect(v)
    rerender()
  }
