...
async function loadSuggestions () {
  suggestions = await dbrowserx.crawler.listSuggestions(query)
  update()
}
...
function renderIcon (row) {
  return yo`<img class="favicon" src="dbrowserx-favicon:32,${row.url}"/>`
}
...
function onContextMenu (e) {
  ...
  async function pin () {
    if (!(await dbrowserx.bookmarks.isBookmarked(url))) {
      await dbrowserx.bookmarks.bookmarkPrivate(url, {title: title})
    }
    await dbrowserx.bookmarks.setBookmarkPinned(url, true)
