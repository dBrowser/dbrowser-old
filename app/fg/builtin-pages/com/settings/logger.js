...
const AVAILABLE_CATEGORIES = ['all', 'crawler', 'dwebx']
...
export default class Logger {
  ...
  async load () {
    ...
    this.rows = await dbrowserx.logger.query({limit: 5e2, filter, until: this.pauseTime, sort: 'desc'})
    this.rows = this.rows.filter(row => this.applyCustomRules(row))

    if (this.readStream) this.readStream.close()
    if (!this.isPaused) {
      this.readStream = dbrowserx.logger.stream({since: Date.now(), filter})
