...
class DriveView extends LitElement {
  ...
  constructor () {
    super()
    this.drive = dbrowserx.ddrive.drive(location)
    this.info = undefined
    this.entries = []
    this.load()
  }
  ...
  renderEntry (entry) {
    if (entry.stat.mount && entry.stat.mount.key) {
      return html`
        <div>
          <a href="dweb://${entry.stat.mount.key}/" title=${entry.name}>
  ...
  async onClickEditProperties (e) {
    e.preventDefault()
    await dbrowserx.shell.drivePropertiesDialog(location.origin)
    this.load()
  }
