...
class SiteInfoApp extends LitElement {
  ...
  get isRootDrive () {
    return this.origin === dbrowser.dwebfs.drive('hyper://system/').url
  }
  ...
  async load () {
    ...
    try {
      ...
      if (this.cert && this.cert.type === 'dwebfs') {
      ...
      this.requestedPerms = await Promise.all(Object.entries(perms).map(async ([perm, value]) => {
        ...
        if (isDatHashRegex.test(permParam)) {
          let driveInfo
          try { driveInfo = await dbrowser.dbrowser.dwebfs.drive(permParam).getInfo() }
          catch (e) { /* ignore */ }
    ...
    return contextMenu.create({
      ...
      items: [
        ...
        this.info && this.info.writable ? {
          ...
          click: async () => {
            await dbrowser.folderSync.configureDialog(this.info.url)
            await dbrowser.browser.refreshTabState()
            dbrowser.browser.executeShellWindowCommand('show-folder-sync-menu')
          }
        } : undefined,
        {
  ...
  async onForkDrive () {
    var drive = await dbrowser.dwebfs.forkDrive(this.url, {detached: false})
