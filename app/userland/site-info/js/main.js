...
class SiteInfoApp extends LitElement {
  ...
  get isDrive () {
    return this.url && this.url.startsWith('dweb:')
  }
  ...
  get isRootDrive () {
    return this.origin === dbrowserx.ddrive.drive('dweb://system/').url
  }
  ...
  constructor () {
    ...
    window.addEventListener('blur', e => {
      dbrowserx.browser.toggleSiteInfo(false)
      this.reset()
    })
      ...
      if (a) {
        ...
        if (href && href !== '#' && !href.startsWith('dbrowser://')) {
          if (isPopup || e.metaKey || a.getAttribute('target') === '_blank') {
            dbrowserx.browser.openUrl(href, {setActive: true})
          } else {
            dbrowserx.browser.gotoUrl(href)
          }
          dbrowserx.browser.toggleSiteInfo(false)
        }
      }
    ...
    try {
      ...
      if (this.isDrive) {
        ...
        ;[this.info, this.driveCfg, this.forks] = await Promise.all([
          drive.getInfo(),
          dbrowserx.drives.get(this.url),
          dbrowserx.drives.getForks(this.url)
        ])
      } else {
        this.info = {
          title: this.hostname,
      ...
      ;[perms, this.cert] = await Promise.all([
        dbrowserx.sitedata.getPermissions(this.origin),
        dbrowserx.browser.getCertificate(this.url)
      ])
      if (this.cert && this.cert.type === 'ddrive') {
      ...
      this.requestedPerms = await Promise.all(Object.entries(perms).map(async ([perm, value]) => {
        ...
        if (isDatHashRegex.test(permParam)) {
          let driveInfo
          try { driveInfo = await dbrowserx.dbrowserx.ddrive.drive(permParam).getInfo() }
          catch (e) { /* ignore */ }
  ...
  async updated () {
    setTimeout(() => {
      ...
      if (!height) return
      dbrowserx.browser.resizeSiteInfo({height})
    }, 50)
  }
  ...
  onChangeUrl (e) {
    ...
    dbrowserx.browser.gotoUrl(this.url)
    this.load()
  }
  ...
  async onClickDriveProperties (e) {
    await dbrowserx.shell.drivePropertiesDialog(this.url)
    this.load()
  }

  async onToggleSaveDrive (e) {
    if (this.driveCfg && this.driveCfg.saved) {
      await dbrowserx.drives.remove(this.origin)
    } else {
      await dbrowserx.drives.configure(this.origin)
    }
    this.load()
  }
    ...
    return contextMenu.create({
      ...
      items: [
        ...
        this.info && this.info.writable ? {
          ...
          click: async () => {
            await dbrowserx.folderSync.syncDialog(this.info.url)
            await dbrowserx.browser.refreshTabState()
          }
        } : undefined,
        {
  ...
  async onForkDrive () {
    var drive = await dbrowserx.ddrive.forkDrive(this.url, {detached: false})
    dbrowserx.browser.openUrl(drive.url, {setActive: true})
  }
