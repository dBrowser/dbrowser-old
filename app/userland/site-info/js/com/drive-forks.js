...
class DriveForks extends LitElement {
  ...
  async onClickNewFork (e) {
    e.preventDefault()
    e.stopPropagation()
    var drive = await dbrowserx.ddrive.forkDrive(this.origin)
  ...
  onClickDiff (e, fork) {
    e.preventDefault()
    e.stopPropagation()
    dbrowserx.browser.openUrl(`dbrowser://diff/?base=${fork.url}&target=${this.forks[0].url}`, {setActive: true})
  ...
  async onClickDelete (e, fork) {
    ...
    await dbrowserx.drives.remove(fork.url)
