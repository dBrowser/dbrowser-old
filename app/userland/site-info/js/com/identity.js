...
class Identity extends LitElement {
  ...
  render () {
    return html`
      ...
      <div class="field-group">
        ${this.cert ? html`
          ${this.cert.type === 'dbrowserx' ? html`
            This is a builtin interface of DBrowserX
          ` : ''}
  ...
  async onToggleSaveContact (e) {
    ...
    if (isContact) {
      await dbrowserx.contacts.remove(this.url)
    } else {
      await dbrowserx.contacts.requestAddContact(this.url)
    }
