...
import datServeResolvePath from '@dbrowser/dwebx-serve-resolve-path'
...
class EditorApp extends LitElement {
  ...
  get drive () {
    return dbrowserx.ddrive.drive(this.url)
  }
  ...
  async load (url, forceLoad = false) {
    ...
    try {
      ...
      if (this.attachedPane && this.attachedPane.url !== this.url) {
        dbrowserx.panes.navigate(this.attachedPane.id, this.url)
      }
      ...
      try {
        if (url.startsWith('dweb:')) {
          ...
          body = await dbrowserx.browser.fetchBody(url)
        } else {
          this.isFilesOpen = false
      ...
      if (!this.dne && !this.isBinary) {
        ...
        if (body && model.getModeId() === 'plaintext') {
          let type = await dbrowserx.browser.getResourceContentType(url)
          if (type) {
  ...
  async loadDrive (url) {
    ...
    // load drive meta
    let drive = dbrowserx.ddrive.drive(url)
    ...
    this.mountInfo = undefined
    {
      ...
      while (pathParts.length) {
        ...
        if (stat && stat.mount) {
          this.mountInfo = await dbrowserx.ddrive.drive(stat.mount.key).getInfo()
  ...
  async showMenu (x, y, folderPath, item, folderItemUrls) {
    ...
    if (item) {
      items.push({
        ...
        click () {
          dbrowserx.browser.openUrl(item.url)
        }
      })
      items.push({
      ...
      if (item.stat.mount && item.stat.mount.key) {
        items.push({
          ...
          click () {
            writeToClipboard(`dweb://${item.stat.mount.key}/`)
            toast.create('Copied to your clipboard')
          }
        })
      }
      ...
      items.push({
        ...
        click: () => {
          dbrowserx.browser.newPane(item.shareUrl, {splitDir: 'vert'})
        }
      })
      items.push({
        label: 'Open in Pane Below',
        click: () => {
          dbrowserx.browser.newPane(item.shareUrl, {splitDir: 'horz'})
        }
      })
      items.push({type: 'separator'})
      items.push({
  ...
  onClickEditReal (e) {
    dbrowserx.browser.openUrl(`dbrowser://editor?url=${this.mountInfo.url + this.mountInfo.resolvedPath}`, {
      setActive: true,
      adjacentActive: true
    })
  }
  ...
  async onClickView () {
    this.attachedPane = dbrowserx.panes.getAttachedPane()
    if (!this.attachedPane) {
      this.attachedPane = await dbrowserx.panes.create(this.url, {attach: true})
    } else {
      dbrowserx.panes.navigate(this.attachedPane.id, this.url)
    }
  }
  ...
  async onClickOpen () {
    var url = await dbrowserx.shell.selectDriveDialog()
  ...
  async onClickImportFiles (folderPath) {
    ...
    try {
      var {numImported} = await dbrowserx.shell.importFilesDialog(joinPath(this.drive.url, folderPath))
  ...
  async onClickImportFolders (folderPath) {
    toast.create('Importing...')
    try {
      var {numImported} = await dbrowserx.shell.importFoldersDialog(joinPath(this.drive.url, folderPath))
      if (numImported > 0) toast.create('Import complete', 'success')
      else toast.destroy()
    } catch (e) {
      console.log(e)
  ...
  async onClickFork (e) {
    var urlp = new URL(this.url)
    var newDrive = await dbrowserx.ddrive.forkDrive(this.url)
    var newDriveUrlp = new URL(newDrive.url)
    urlp.hostname = newDriveUrlp.hostname
    
    dbrowserx.browser.gotoUrl(urlp.toString())
    this.load(urlp.toString())
  }
}
