...
class WebTerm extends LitElement {
  ...
  constructor () {
      ...
      if (anchor) {
        ...
        if (e.metaKey || anchor.getAttribute('target') === '_blank') {
          ...
          if (this.attachedPane) {
            dbrowserx.panes.navigate(this.attachedPane.id, anchor.getAttribute('href'))
          } else {
    ...
    ;(async () => {
      ...
      this.load(ctx || 'dweb://system/').then(_ => {
        this.setFocus()
      })
    })()
  }
  ...
  async loadCommands () {
    var packages = [{
      ...
      manifest: JSON.parse(await dbrowserx.browser.readFile('dbrowser://std-cmds/index.json', 'utf8'))
    }]

    var cmdPkgDrives = await dbrowserx.ddrive.readFile('dweb://system/webterm/command-packages.json').then(JSON.parse).catch(e => ([]))
    for (let driveUrl of cmdPkgDrives) {
      try {
        packages.push({
          url: driveUrl,
          manifest: JSON.parse(await dbrowserx.ddrive.drive(driveUrl).readFile(`index.json`))
        })
      } catch (e) {
        console.log(e)
  ...
  async loadPageCommands () {
    ...
    this.pageCommands = await dbrowserx.panes.executeJavaScript(this.attachedPane.id, `
      ;(() => {
        let commands = {}
        if (dbrowserx.terminal.getCommands().length) {
          for (let command of dbrowserx.terminal.getCommands()) {
  ...
  setCWD (location) {
    ...
    if (location.startsWith('dweb://')) {
      ...
      locationParsed = new URL('dweb://' + location)
    } else {
      locationParsed = new URL(location)
    }
    ...
    if (this.attachedPane && this.attachedPane.url !== locationParsed.toString()) {
      dbrowserx.panes.navigate(this.attachedPane.id, locationParsed.toString())
    }
    this.requestUpdate()
  }

  parseURL (url) {
    if (!url.includes('://')) url = 'dweb://' + url
    return new URL(url)
  }
  ...
  async evalPrompt () {
    ...
    if (commandName.startsWith('@')) {
      ...
      if (command) {
        command.fn = (...args) => dbrowserx.panes.executeJavaScript(this.attachedPane.id, `
          ;(() => {
            let command = dbrowserx.terminal.getCommands().find(c => c.name === ${JSON.stringify(commandName.slice(1))});
            if (command) {
        ...
        page: {
          goto (url, opts = {}) {
            if (opts.newTab) window.open(url)
              ...
              var pane = dbrowserx.panes.getAttachedPane()
              if (!pane) throw new Error('No attached pane')
              dbrowserx.panes.navigate(pane.id, url)
            }
          },
          refresh () {
            var pane = dbrowserx.panes.getAttachedPane()
            if (!pane) throw new Error('No attached pane')
            dbrowserx.panes.navigate(pane.id, pane.url)
          },
          focus () {
            var pane = dbrowserx.panes.getAttachedPane()
            if (!pane) throw new Error('No attached pane')
            dbrowserx.panes.focus(pane.id)
          },
          exec (js) {
            var pane = dbrowserx.panes.getAttachedPane()
            if (!pane) throw new Error('No attached pane')
            return dbrowserx.panes.executeJavaScript(pane.id, js)
          },
          inject (css) {
            var pane = dbrowserx.panes.getAttachedPane()
            if (!pane) throw new Error('No attached pane')
            return dbrowserx.panes.injectCss(pane.id, css)
          },
          uninject (id) {
            var pane = dbrowserx.panes.getAttachedPane()
            if (!pane) throw new Error('No attached pane')
            return dbrowserx.panes.uninjectCss(pane.id, id)
          }
        },
        out: (...args) => {
          args = args.map(arg => {
