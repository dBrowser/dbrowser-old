...
export class ExplorerApp extends LitElement {
  ...
  constructor () {
    ...
    dbrowserx.panes.addEventListener('pane-attached', e => {
      this.attachedPane = dbrowserx.panes.getAttachedPane()
      if (loc.getUrl() !== this.attachedPane.url) {
        loc.setUrl(this.attachedPane.url)
      }
    })
    dbrowserx.panes.addEventListener('pane-detached', e => {
      this.attachedPane = undefined
    })
    dbrowserx.panes.addEventListener('pane-navigated', e => {
    ...
    ;(async () => {
      ...
      if (this.attachedPane) {
        ...
        if (loc.getUrl() && loc.getUrl() !== this.attachedPane.url) {
          dbrowserx.panes.navigate(this.attachedPane.id, loc.getUrl())
        }
      } else {
        // try to attach and then follow their url
        this.attachedPane = await dbrowserx.panes.attachToLastActivePane()
  ...
  async load () {
    if (this.attachedPane) {
      if (!loc.getUrl() || !loc.getUrl().startsWith('dweb://')) {
        this.errorState = {task: 'Attempting to load', error: new Error('Failed to load this address: not a ddrive')}
        return
      }
    } else if (!loc.getUrl()) {
      return loc.setUrl('dweb://system/')
    }

    // read helper state
    dbrowserx.ddrive.readFile('dweb://system/address-book.json', 'json').then(addressBook => {
      return Promise.all(addressBook.profiles.map(p => dbrowserx.ddrive.getInfo(p.key)))
    }).then(profiles => {
      profiles.sort((a, b) => (a.title||'').localeCompare(b.title||''))
      this.profiles = profiles
      return dbrowserx.drives.list()
    }).then(drives => {
    ...
    var drive = dbrowserx.ddrive.drive(loc.getOrigin())
    try {
    ...
    if (this.pathInfo.isDirectory()) {
      ...
      if (!this.watchStream) {
        let currentDrive = dbrowserx.ddrive.drive(this.currentDriveInfo.url)
    ...
    while (pathParts.length) {
      ...
      if (stat.mount) {
        mount = await this.attempt(
          ...
          () => dbrowserx.ddrive.drive(stat.mount.key).getInfo()
        )
      }
    ...
    for (let item of items) {
      ...
      if (item.stat.mount) {
        item.mount = await this.attempt(
          ...
          () => dbrowserx.ddrive.drive(item.stat.mount.key).getInfo()
        )
      }
  ...
  async readViewfile (drive) {
    ...
    await this.attempt(
      ...
      item.url = joinPath(loc.getOrigin(), item.path)
      item.shareUrl = this.getShareUrl(item)
      item.drive = await dbrowserx.ddrive.drive(item.drive).getInfo()
      item.mount = item.mount ? dbrowserx.ddrive.drive(item.mount).getInfo() : undefined
      this.setItemIcons(item)
    })))
  ...
  getShareUrl (item) {
    if (item.stat.mount) {
      return `dweb://${item.stat.mount.key}`
  ...
  goto (item, newWindow = false, leaveExplorer = false) {
    ...
    if (this.attachedPane) {
      ...
      else dbrowserx.panes.navigate(this.attachedPane.id, url)
    } else {
      if (leaveExplorer) {
        if (newWindow) window.open(url)
  ...
  async onNewFile (e) {
    ...
    if (filename) {
      ...
      var drive = dbrowserx.ddrive.drive(this.currentDriveInfo.url)
  ...
  async onNewMount (e) {
    if (!this.currentDriveInfo.writable) return
    var drive = dbrowserx.ddrive.drive(this.currentDriveInfo.url)
    var targetUrl = await dbrowserx.shell.selectDriveDialog({title: 'Select a drive'})
    var target = dbrowserx.ddrive.drive(targetUrl)
  ...
  async onForkDrive (e) {
    var drive = await dbrowserx.ddrive.forkDrive(this.currentDriveInfo.url)
    toast.create('Drive created')
    loc.setUrl(drive.url)
  }

  async onDriveProperties (e) {
    await dbrowserx.shell.drivePropertiesDialog(this.currentDriveInfo.url)
    this.load()
  }
  ...
  async onImportFiles (e) {
    ...
    try {
      var {numImported} = await dbrowserx.shell.importFilesDialog(loc.getUrl())
  ...
  async onImportFolders (e) {
    ...
    toast.create('Importing...')
    try {
      var {numImported} = await dbrowserx.shell.importFoldersDialog(loc.getUrl())
      if (numImported > 0) toast.create('Import complete', 'success')
      else toast.destroy()
    } catch (e) {
      console.log(e)
  ...
  async onExport (e) {
    ...
    try {
      var {numExported} = await dbrowserx.shell.exportFilesDialog(urls)
  ...
  async onShowMenu (e, useAppMenuAlways = false) {
    ...
    if (!useAppMenuAlways && typeof dbrowserx !== 'undefined' && typeof dbrowserx.browser !== 'undefined') {
      let fns = {}
      ...
      var choice = await dbrowserx.browser.showContextMenu(items, true)
  ...
  async doDiff (base) {
    if (this.attachedMode) dbrowserx.browser.gotoUrl(`dbrowser://diff/?base=${base}`)
    else window.open(`dbrowser://diff/?base=${base}`)
  }

  async onSelectDrive (e) {
    e.preventDefault()
    var drive = await dbrowserx.shell.selectDriveDialog()
    loc.setUrl(drive)
  }
}
