import { LitElement, html, css } from '/vendor/dbrowserx-app-stdlib/vendor/lit-element/lit-element.js'
import * as appMenu from '/vendor/dbrowserx-app-stdlib/js/com/app-menu.js'
import * as contextMenu from '/vendor/dbrowserx-app-stdlib/js/com/context-menu.js'
import * as toast from '/vendor/dbrowserx-app-stdlib/js/com/toast.js'
...
class TopRightControls extends LitElement {
  ...
  onClickNewMenu (e) {
    ...
    async function create (templateUrl, title, description, urlModifyFn) {
      ...
      try {
        var newSite = await dbrowserx.ddrive.forkDrive(templateUrl, {title, description, prompt: false})
        window.location = urlModifyFn ? urlModifyFn(newSite.url) : newSite.url
      } catch (e) {
        console.error(e)
        if (e.name === 'TimeoutError') {
          toast.create('DBrowserX was unable to download the template for your new site. Please check your Internet connection and try again!', 'error')
        } else {
  ...
  onClickProfileMenu (e) {
    ...
    const items = [
      ...
      {icon: false, label: 'DBrowserX.Social profile', click: () => goto(`intent:unwalled.garden/view-feed?url=${encodeURIComponent(this.userUrl)}`)},
      '-',
...
customElements.define('dbrowserx-top-right-controls', TopRightControls)
