...
class TopRightControls extends LitElement {
  ...
  onClickNewMenu (e) {
    ...
    async function create (templateUrl, title, description, urlModifyFn) {
      ...
      try {
        var newSite = await dbrowser.dwebfs.forkDrive(templateUrl, {title, description, prompt: false})
  ...
  onClickProfileMenu (e) {
    ...
    const items = [
      ...
      {icon: false, label: 'dBrowser.Social profile', click: () => goto(`intent:unwalled.garden/view-feed?url=${encodeURIComponent(this.userUrl)}`)},
      '-',
