      ...
      return html`
        ...
        <div class="dropdown-items roomy no-border">
          ...
          <div class="dropdown-item" @click=${onCreateDriveFromFolder}>
            ...
            <div class="hover-help">
            <span class="fas fa-info"></span> <p>Create a ddrive by importing from a folder on your computer.</p>
            </div>
          </div>
        </div>
      `
...
async function onCreateDrive (type) {
  contextMenu.destroy()
  var drive = await dbrowserx.ddrive.createDrive({type})
  window.location = drive.url
}

async function onCreateDriveFromFolder () {
  contextMenu.destroy()
  var folder = await dbrowserx.browser.showOpenDialog({
    title: 'Select folder',
  ...
  await dbrowserx.ddrive.importFromFilesystem({src: folder[0], dst: drive.url})
  window.location = drive.url
}