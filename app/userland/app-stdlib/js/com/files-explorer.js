...
class FilesExplorer extends LitElement {
  ...
  onContextmenuItem (e, item) {
    ...
    items.push({
      ...
      click: () => {
        dbrowserx.browser.openUrl(url, {setActive: true})
      }
    })
    if (item.stat.isFile()) {
      items.push({
        ...
        label: `Edit file`,
        click: () => {
          dbrowserx.browser.openUrl(`dbrowser://editor/?url=${url}`, {setActive: true})
        }
      })
    }
    items.push({
    ...
    items.push({
      ...
      click: async () => {
        ...
        if (oldpath === this.pathname) {
          dbrowserx.browser.gotoUrl(joinPath(this.origin, newpath))
        } else {
          this.load()
        }
      }
    ...
    if (item.stat.mount) {
      items = items.concat([
        '-',
        {
          ...
          click: () => {
            dbrowserx.browser.openUrl(`dweb://${item.stat.mount.key}/`, {setActive: true})
          }
        },
        {
          icon: 'fas fa-fw fa-link',
    ...
    if (this.currentFolder.mount) {
      items = items.concat([
        '-',
        {
          ...
          label: `Open mount in new tab`,
          click: () => {
            dbrowserx.browser.openUrl(`dweb://${this.currentFolder.mount.key}/`, {setActive: true})
          }
        },
        {
          icon: 'fas fa-fw fa-link',
          label: `Copy Mount URL`,
          click () {
            writeToClipboard(`dweb://${this.currentFolder.mount.key}/`)
            toast.create('Copied to your clipboard')
          }
        }
      ])
  ...
  async onClickImportFiles (e) {
    ...
    let browserInfo = await dbrowserx.browser.getInfo()
    ...
    if (files) {
      for (let src of files) {
        await dbrowserx.ddrive.importFromFilesystem({
          src,
  ...
  async onClickExportFiles (e) {
    dbrowserx.browser.downloadURL(`${this.origin}?download_as=zip`)
  }
