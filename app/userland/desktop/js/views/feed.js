...
import { unsafeHTML } from 'dbrowser://app-stdlib/vendor/lit-element/lit-html/directives/unsafe-html.js'
import MarkdownIt from 'dbrowser://app-stdlib/vendor/markdown-it.js'
...
export class FeedView extends LitElement {
  ...
  async getContacts () {
    var contacts = await dbrowserx.contacts.list()
    if (!this.profile) {
  ...
  async loadFeed (fresh = false) {
    ...
    var files = await dbrowserx.ddrive.query({
      path: '/microblog/*',
    ...
    for (let file of files) {
      ...
      try {
        ...
        if (/\.(png|jpe?g|gif|svg)$/i.test(file.path)) {
          ...
          let txt = await dbrowserx.ddrive.readFile(file.url)
          // render content
