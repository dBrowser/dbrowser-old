...
const RELEASES = [
  { label: '1.0, Beta 7', url: 'https://dbrowser.com/2020/07/15/dbrowserx-1-0-beta-7.html' },
  { label: '1.0, Beta 6', url: 'https://dbrowser.com/2020/07/10/dbrowserx-1-0-beta-6.html' },
  { label: '1.0, Beta 5', url: 'https://dbrowser.com/2020/06/19/dbrowserx-1-0-beta-5.html' },
  { label: '1.0, Beta 4', url: 'https://dbrowser.com/2020/06/04/dbrowserx-1-0-beta-4.html' },
  { label: '1.0, Beta 3', url: 'https://dbrowser.com/2020/05/28/dbrowserx-1-0-beta-3.html' },
  { label: '1.0, Beta 2', url: 'https://dbrowser.com/2020/05/20/dbrowserx-1-0-beta-2.html' },
  { label: '1.0, Beta 1', url: 'https://dbrowser.com/2020/05/14/dbrowserx-1-0-beta.html' }
]
...
class DesktopApp extends LitElement {
  ...
  async load () {
    ...
    this.legacyArchives = await dbrowserx.datLegacy.list()
  }
  ...
  renderIntro () {
    ...
    return html`
      <div class="intro">
          ...
          <div>
            ${this.profile ? html`
              <a href=${this.profile.url} target="_blank">
                <dbrowserx-img-fallbacks class="avatar">
                  <img src="${this.profile.url}/thumb?cache_buster=${cacheBuster}" slot="img1">
                  <img src="dbrowser://assets/default-user-thumb" slot="img2">
                </dbrowserx-img-fallbacks>
              </a>
        ...
        <div class="col1">
          <a class="icon" href="${DOCS_URL}/getting-started-with-dbrowserx" target="_blank">
            <span class="fas fa-book"></span>
          </a>
          <h4>4. Read the <a href="${DOCS_URL}/getting-started-with-dbrowserx" target="_blank">Getting Started Guide</a>.</h4>
        </div>
      </div>
    `
  }
  ...
  renderLegacyArchivesNotice () {
    ...
    return html`
      <div class="legacy-archives-notice">
        <details>
          <summary>You have ${this.legacyArchives.length} legacy DWebX ${pluralize(this.legacyArchives.length, 'archive')} which can be converted to DDrive.</summary>
          <div class="archives">
          ${this.legacyArchives.map(archive => html`
            <div class="archive">
              <a href="dwebx://${archive.key}" title=${archive.title} target="_blank">${archive.title || archive.key}</a>
              <button @click=${e => {window.location = `dwebx://${archive.key}`}}>View</button>
  ...
  async onClickNewPost (e) {
    try {
      ...
      await dbrowserx.ddrive.drive(this.profile.url).writeFile(joinPath('microblog', post.filename), post.body)
  ...
  async onClickNewDrive (e) {
    var drive = await dbrowserx.ddrive.createDrive()
  ...
  async onContextmenuFile (e, file) {
    ...
    var choice = await dbrowserx.browser.showContextMenu(items)
  ...
  async onClickRemove (file) {
    await dbrowserx.ddrive.deleteMetadata(`dweb://system/bookmarks/${file.name}`, 'pinned')
  ...
  async onClickRemoveLegacyArchive (e, archive) {
    ...
    await dbrowserx.datLegacy.remove(archive.key)
...
function getHref (file) {
  ...
  return `dweb://system/bookmarks/${file.name}`
}
