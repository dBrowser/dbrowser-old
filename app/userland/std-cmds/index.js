          ...
          ${listing
            ...
            .map(entry => {
              ...
              const mountInfo = entry.stat.mount
                ? html` <a href="dweb://${entry.stat.mount.key}/" class="color-lightgray" style="font-weight: lighter">(<term-icon solid fw icon="external-link-square-alt"></term-icon>${entry.stat.mount.key.slice(0, 4)}..${entry.stat.mount.key.slice(-2)})</a>`
                : ''
...
export async function cd (opts = {}, location = '') {
  ...
  if (cwd.startsWith('dweb://')) {
    ...
    let drive = dbrowserx.ddrive.drive(urlp.origin)
    let st
...
export async function bookmark (opts = {}, href = '.') {
  ...
  await dbrowserx.ddrive.drive('dweb://system/').writeFile(`/bookmarks/${name}`, '', {metadata: {href}})
}
...
export async function open (opts = {}, location = '') {
  if (opts.bookmark) {
    location = `${dbrowserx.ddrive.getSystemDrive().url}/bookmarks/${location}`
...
export const system = {
  async fs_audit_stream (opts = {}) {
    ...
    var stream = await dbrowserx.logger.streamAuditLog()
    stream.addEventListener('data', e => {
      ...
      html.render(html`
        ...
        <div><small style="padding: 0 5px; width: 50px; display: inline-block">target</small> <a href="dweb://${e.detail.target}/">dweb://${e.detail.target}</a></div>
