...
export async function cd (opts = {}, location = '') {
  ...
  if (cwd.startsWith('hyper://')) {
    ...
    let drive = dbrowser.dwebfs.drive(urlp.origin)
    let st
...
export async function bookmark (opts = {}, href = '.') {
  ...
  await dbrowser.dwebfs.drive('hyper://system/').writeFile(`/bookmarks/${name}`, '', {metadata: {href}})
}
...
export async function open (opts = {}, location = '') {
  if (opts.bookmark) {
    location = `${dbrowser.dwebfs.getSystemDrive().url}/bookmarks/${location}`
