...
export async function ls () {
  ...
  for (let drive of drives) {
    driveInfos.push(await dbrowser.dwebfs.drive(drive).getInfo())
  }
  driveInfos.toHTML = () => {
...
async function readInstalled () {
  return dbrowser.dwebfs.drive('hyper://system/').readFile('/webterm/command-packages.json').then(JSON.parse).catch(e => ([]))
}

async function saveInstalled (urls) {
  await dbrowser.dwebfs.drive('hyper://system/').mkdir('/webterm').catch(e => undefined)
  await dbrowser.dwebfs.drive('hyper://system/').writeFile('/webterm/command-packages.json', JSON.stringify(urls, null, 2))
}
