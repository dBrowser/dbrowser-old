...
export async function ls () {
  ...
  for (let drive of drives) {
    driveInfos.push(await dbrowserx.ddrive.drive(drive).getInfo())
  }
  driveInfos.toHTML = () => {
...
async function readInstalled () {
  return dbrowserx.ddrive.drive('dweb://system/').readFile('/webterm/command-packages.json').then(JSON.parse).catch(e => ([]))
}

async function saveInstalled (urls) {
  await dbrowserx.ddrive.drive('dweb://system/').mkdir('/webterm').catch(e => undefined)
  await dbrowserx.ddrive.drive('dweb://system/').writeFile('/webterm/command-packages.json', JSON.stringify(urls, null, 2))
}
