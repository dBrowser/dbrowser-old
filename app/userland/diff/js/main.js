...
export class CompareApp extends LitElement {
  ...
  async load () {
    ...
    this.baseDrive = this.base ? dbrowser.dwebfs.drive(this.base) : undefined
    this.targetDrive = this.target ? dbrowser.dwebfs.drive(this.target) : undefined
    await this.attempt(
...
class CompareDiffItemContent extends LitElement {
  ...
  renderLeftColumn () {
    if (this.diff.change === 'del' || this.diff.change === 'mod') {
      return this.renderFileContent(dbrowser.dwebfs.drive(this.targetOrigin), this.diff.targetPath, this.diff.targetMountKey)
    }
    return ''
  }
  ...
  renderRightColumn () {
    if (this.diff.change === 'add' || this.diff.change === 'mod') {
      return this.renderFileContent(dbrowser.dwebfs.drive(this.baseOrigin), this.diff.basePath, this.diff.baseMountKey)
    }
    return ''
  }
  ...
  async updated () {
    ...
    var [baseContent, targetContent] = await Promise.all([
      this.diff.change === 'del' || this.diff.change === 'mod' ? (dbrowser.dwebfs.drive(this.targetOrigin)).readFile(this.diff.targetPath).catch(e => '') : '',
      this.diff.change === 'add' || this.diff.change === 'mod' ? (dbrowser.dwebfs.drive(this.baseOrigin)).readFile(this.diff.basePath).catch(e => '') : '',
    ])
