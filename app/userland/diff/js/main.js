...
export class CompareApp extends LitElement {
  ...
  async load () {
    if (!this.otherDrives) {
      ;(async () => {
        this.otherDrives = await dbrowserx.drives.list({includeSystem: false})
    ...
    this.baseDrive = this.base ? dbrowserx.ddrive.drive(this.base) : undefined
    this.targetDrive = this.target ? dbrowserx.ddrive.drive(this.target) : undefined
    await this.attempt(
    ...
    this.baseForks = this.baseInfo
      ? await this.attempt(
        ...
        () => dbrowserx.drives.getForks(this.baseInfo.url)
       ) : undefined
    this.targetForks = this.targetInfo
      ? await this.attempt(
        'Fetching known forks of the target drive',
        () => dbrowserx.drives.getForks(this.targetInfo.url)
       ) : undefined
    ...
    contextMenu.create({
      ...
      items: this.otherDrives.slice(0, 10).map(drive => ({
        ...
        '-',
        {
          ...
          label: 'Browse...',
          click: async () => {
            this.base = await dbrowserx.shell.selectDriveDialog()
            this.load()
          }
        }
      ])
    ...
    contextMenu.create({
      ...
      items: this.otherDrives.slice(0, 10).map(drive => ({
        ...
        '-',
        {
          ...
          label: 'Browse...',
          click: async () => {
            this.target = await dbrowserx.shell.selectDriveDialog()
            this.load()
          }
        }
      ])
...
class CompareDiffItemContent extends LitElement {
  ...
  renderLeftColumn () {
    if (this.diff.change === 'del' || this.diff.change === 'mod') {
      return this.renderFileContent(dbrowserx.ddrive.drive(this.targetOrigin), this.diff.targetPath, this.diff.targetMountKey)
    }
    return ''
  }
  ...
  renderRightColumn () {
    if (this.diff.change === 'add' || this.diff.change === 'mod') {
      return this.renderFileContent(dbrowserx.ddrive.drive(this.baseOrigin), this.diff.basePath, this.diff.baseMountKey)
    }
    return ''
  }
  ...
  async updated () {
    ...
    var [baseContent, targetContent] = await Promise.all([
      this.diff.change === 'del' || this.diff.change === 'mod' ? (dbrowserx.ddrive.drive(this.targetOrigin)).readFile(this.diff.targetPath).catch(e => '') : '',
      this.diff.change === 'add' || this.diff.change === 'mod' ? (dbrowserx.ddrive.drive(this.baseOrigin)).readFile(this.diff.basePath).catch(e => '') : '',
    ])
