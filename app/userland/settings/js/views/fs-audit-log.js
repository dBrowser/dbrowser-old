...
class FsAuditLogView extends LitElement {
  ...
  async load () {
    ...
    this.rows = await dbrowserx.logger.listAuditLog({keys: [], limit: 5e3})

    this.isLoading = false
    this.requestUpdate()

    this.stats = await dbrowserx.logger.getAuditLogStats()
    this.requestUpdate()
  }
          ...
          <tbody>${this.rows.map((row, i) => {
            ...
            return html`
              ...
              <tr class="logger-row ${row.runtime > 250 ? 'badish' : ''} ${row.runtime > 1e3 ? 'bad' : ''}">
                ...
                <td class="target"><a href=${'dweb://' + row.target} target="_blank">${toNiceDomain(row.target)}</a></td>
