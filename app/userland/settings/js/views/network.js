...
class NetworkView extends LitElement {
  ...
  async load () {
    this.error = undefined
    try {
      var networkStatus = await dbrowserx.browser.getDaemonNetworkStatus()
      for (let stats of networkStatus) {
        stats[0].drive = await dbrowserx.drives.get(stats[0].metadata.key)
      }
  ...
  renderDriveStatus (stats) {
    ...
    return html`
      <tr>
        <td>
          <a href="dweb://${drive && drive.ident.system ? 'system' : key}/" target="_blank">
            ${title}
            ${forkOf}
          </a>
        </td>
