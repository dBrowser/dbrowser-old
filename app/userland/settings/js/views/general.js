...
class GeneralSettingsView extends LitElement {
  ...
  async load () {
    // wire up events
    this.browserEvents = dbrowserx.browser.createEventsStream()
    ...
    // fetch data
    this.browserInfo = await dbrowserx.browser.getInfo()
    this.settings = await dbrowserx.browser.getSettings()
    this.defaultProtocolSettings = await dbrowserx.browser.getDefaultProtocolSettings()
    console.log('loaded', {
    ...
    if (this.browserInfo && !this.browserInfo.isDaemonActive) {
      return html`
        <div class="section warning">
          ...
          <p>
            The "daemon" runs DBrowserX's DDrive networking.
          </p>
          <p>
  ...
  renderAnalyticsSettings () {
    const toggle = () => {
      ...
      dbrowserx.browser.setSetting('analytics_enabled', this.settings.analytics_enabled)
      this.requestUpdate()
      toast.create('Setting updated')
    }
  ...
  onToggleAutoUpdate () {
    ...
    this.requestUpdate()
    dbrowserx.browser.setSetting('auto_update_enabled', this.settings.auto_update_enabled)
    toast.create('Setting updated')
  }

  onCustomStartPageChange (e) {
    this.settings.custom_start_page = e.target.value
    dbrowserx.browser.setSetting('custom_start_page', this.settings.custom_start_page)
    toast.create('Setting updated')
  }

  onBrowserThemeChange (e) {
    this.settings.browser_theme = e.target.value
    dbrowserx.browser.setSetting('browser_theme', this.settings.browser_theme)
    toast.create('Setting updated')
  }

  onNewTabsInForegroundToggle (e) {
    this.settings.new_tabs_in_foreground = this.settings.new_tabs_in_foreground == 1 ? 0 : 1
    dbrowserx.browser.setSetting('new_tabs_in_foreground', this.settings.new_tabs_in_foreground)
    toast.create('Setting updated')
  }

  onRunBackgroundToggle (e) {
    this.settings.run_background = this.settings.run_background == 1 ? 0 : 1
    dbrowserx.browser.setSetting('run_background', this.settings.run_background)
    toast.create('Setting updated')
  }

  onNewTabChange (e) {
    this.settings.new_tab = e.target.value
    dbrowserx.browser.setSetting('new_tab', this.settings.new_tab)
    toast.create('Setting updated')
  }

  async onClickBrowseNewTab (e) {
    var sel = await dbrowserx.shell.selectFileDialog({
      allowMultiple: false
    })
    if (sel) {
  ...
  onSearchEngineChange (e) {
    ...
    dbrowserx.browser.setSetting('search_engines', this.settings.search_engines)
    toast.create('Setting updated')
    this.requestUpdate()
  }
  ...
  onChangeDefaultZoom (e) {
    ...
    dbrowserx.browser.setSetting('default_zoom', this.settings.default_zoom)
    toast.create('Setting updated')
  }
  ...
  async onClickRestartDaemon (e) {
    ...
    await dbrowserx.browser.reconnectHyperdriveDaemon()
    this.browserInfo = await dbrowserx.browser.getInfo()
    this.requestUpdate()
  }
}
