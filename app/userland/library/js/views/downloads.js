...
export class DownloadsView extends LitElement {
  ...
  async load () {
    this.downloads = await dbrowserx.downloads.getDownloads()
    console.log(this.downloads)

    var dlEvents = dbrowserx.downloads.createEventsStream()
  ...
  onPauseDownload (download) {
    dbrowserx.downloads.pause(download.id)
  }

  onResumeDownload (download) {
    dbrowserx.downloads.resume(download.id)
  }

  onCancelDownload (download) {
    dbrowserx.downloads.cancel(download.id)
  }
  ...
  onShowDownload (download) {
    dbrowserx.downloads.showInFolder(download.id)
      .catch(err => {
  ...
  onOpenDownload (download) {
    dbrowserx.downloads.open(download.id)
      .catch(err => {
        download.fileNotFound = true
        this.requestUpdate()
      })
  ...
  onRemoveDownload (download) {
    dbrowserx.downloads.remove(download.id)
