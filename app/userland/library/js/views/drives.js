...
const EXPLORER_URL = drive => `dbrowser://explorer/${drive.url.slice('hyper://'.length)}`
...
export class DrivesView extends LitElement {
  ...
  async forkDrive (drive) {
    var drive = await dbrowser.dwebfs.forkDrive(drive.url)
      ...
      ${drives ? html`
        ...
        <div class="drives">
          ...
          ${drives.length === 0 && !this.hideEmpty ? html`
            ...
            ` : html`
              <div class="empty"><span class="fas fa-hdd"></span><div>Click "New Drive" to create a DWebFs</div></div>
            `}
          ` : ''}
