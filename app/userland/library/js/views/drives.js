...
export class DrivesView extends LitElement {
  ...
  async load () {
    var drives = await dbrowserx.drives.list({includeSystem: false})
  ...
  async forkDrive (drive) {
    var drive = await dbrowserx.ddrive.forkDrive(drive.url)
  ...
  async driveProps (drive) {
    await dbrowserx.shell.drivePropertiesDialog(drive.url)
    this.load()
  }

  async removeDrive (drive) {
    await dbrowserx.drives.remove(drive.url)
    const undo = async () => {
      await dbrowserx.drives.configure(drive.url)
