...
export class BookmarksView extends LitElement {
  ...
  async load () {
    var bookmarks = await dbrowserx.ddrive.drive('dweb://system/').query({
      type: 'file',
      path: ['/bookmarks/*.goto']
    })
    var toolbar = await dbrowserx.ddrive.readFile('dweb://system/toolbar.json', 'json').catch(e => undefined)
  ...
  async onToggleBookmarkPinned (e, bookmark) {
    ...
    if (_pinned(bookmark)) {
      await dbrowserx.ddrive.drive('dweb://system/').deleteMetadata(bookmark.path, ['pinned'])
    } else {
      await dbrowserx.ddrive.drive('dweb://system/').updateMetadata(bookmark.path, {pinned: '1'})
    }
    this.load()
  ...
  async onToggleBookmarToolbar (bookmark) {
    ...
    await dbrowserx.ddrive.writeFile('dweb://system/toolbar.json', toolbar, 'json')
    dbrowserx.browser.updateWindowToolbar()
    this.load()
  }
  ...
  async onClickRemove (file) {
    ...
    await dbrowserx.ddrive.drive('dweb://system/').unlink(file.path)
