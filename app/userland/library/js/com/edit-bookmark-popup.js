...
export class EditBookmarkPopup extends BasePopup {
    ...
    return html`
      ...
      <form @submit=${this.onSubmit}>
        <div>
          ...
          <input required type="text" id="title-input" name="title" value="${this.bookmark?.stat.metadata.title || ''}" placeholder="E.g. DBrowserX Browser" />
  ...
  async onSubmit (e) {
    ...
    if (this.bookmark) {
      await dbrowserx.ddrive.drive('dweb://system/').updateMetadata(this.bookmark.path, {
        href: e.target.href.value,
        title: e.target.title.value,
        pinned: pinned ? '1' : undefined
      })
      if (!pinned) await dbrowserx.ddrive.drive('dweb://system/').deleteMetadata(this.bookmark.path, ['pinned'])
    } else {
      await dbrowserx.bookmarks.add({
        href: e.target.href.value,
        title: e.target.title.value,
        pinned
      })
