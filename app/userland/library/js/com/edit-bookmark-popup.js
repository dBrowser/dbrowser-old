...
export class EditBookmarkPopup extends BasePopup {
  ...
  async onSubmit (e) {
    ...
    if (this.bookmark) {
      await dbrowser.dwebfs.drive('hyper://system/').updateMetadata(this.bookmark.path, {
        href: e.target.href.value,
        title: e.target.title.value,
        pinned: pinned ? '1' : undefined
      })
      if (!pinned) await dbrowser.dwebfs.drive('hyper://system/').deleteMetadata(this.bookmark.path, ['pinned'])
    } else {
