...
export class AddContactPopup extends BasePopup {
  ...
  async loadContactInfo () {
    ...
    var error
    try {
      info = await dbrowserx.ddrive.getInfo(this.urlValue)
      if (info.version === 0) {
        info = undefined
        error = 'Unable to find this ddrive on the network'
      }
    } catch (e) {
  ...
  async onSubmit (e) {
    ...
    const sysDrive = dbrowserx.ddrive.drive('dweb://system/')
    ...
    if (this.hostChecked) {
      await dbrowserx.drives.configure(this.contactInfo.key)
    }
