...
export function renderPermDesc ({html, bg, url, permId, permParam, permOpts}) {
  ...
  const openUrl = url => e => {
    ...
    else dbrowserx.browser.openUrl(url, {setActive: true})
  }
  ...
  switch (permId) {
    ...
    case 'contactsList': return 'Read your address-book in DBrowserX'
    ...
    case 'createDrive':
      if (permOpts.title) return `Create a new DDrive, "${permOpts.title}"`
      return 'Create a new DDrive'
